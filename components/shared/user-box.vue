    ...
    <template #panel>
      <div class="p-4 w-72 bg-white dark:bg-black">
        ...
        <div class="flex items-center mt-3 space-x-2">
          <div class="rounded-md dark:bg-gray-900 bg-gray-200 p-1">
            <UAvatar :alt="currentUser.name" class="uppercase" />
          </div>
          <div class="capitalize">{{ currentUser.name }}.s documents</div>
        </div>
        <UDivider class="my-3" />
        <NuxtLink to="profile" class="mx-1">
          <UButton>profile</UButton>
        </NuxtLink>
        <UButton class="font-bold" color="rose" @click="logout"
          >Log Out</UButton
        >
      </div>
    </template>
